...
// type AppointmentFormValues = z.infer<typeof walkinSchema>;
...
export default function CheckIn() {
    ...
    // Walk-in email verification handlers
    const handleSendVisitorOtp = async () => {
        try {
            const email = watch("email");
            if (!email) return;

            await sendVisitorOtp({ email }).unwrap();
            setVisitorOtpSent(true);
        } catch (err) {
            console.error(err);
        }
    ...
    const handleResendVisitorOtp = async () => {
        try {
            setVisitorResendLoading(true);
            await sendVisitorOtp({ email: watch("email") }).unwrap();
            setVisitorOtp(["", "", "", "", "", ""]);
            setVisitorResendTimer(30);
        } catch (err) {
            console.error(err);
        ...
        <section className="mb-10">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
                ...
                >
                                ...
                                {walkinStep === "form" && (
                                    <div className="border-t border-gray-200 max-w-xl mt-4 py-4 flex gap-2">
                                        <Button
                                            type="submit"
                                            className="bg-maroon hover:bg-maroon-dark"
                                            disabled={isLoading || !visitorEmailVerified}
                                        >
                    ...
                    {/* PRE-SCHEDULE TAB */}
                    {activeTab === "preschedule" && isSuperAdmin && (
                        <PreScheduleForm />
                    )}
                </div>
            </div>
